<?php
	...
	/*
		...
		Plugin Name: Social Graph Protocol Buddypress YR
		Plugin URI:		http://www.faxservice.com.ua/wp/ads/social-graph-protocol-buddypress-yr-plugin/

		Description: It is a modified "Social Graph Protocol" Plugin. Adds Open Graph meta tags to your Wordpress website and into Buddypress Activity pages. Adds URL tag with redirected Buddypress Activity URL and Description tag, that enables Facebook Sharer algoritm do right. It is recommended to use with Facebook Share plugins for Buddypress.
		...
		License: <a href="http://www.gnu.org/licenses/old-licenses/gpl-2.0.html" target="_blank">GPL2</a>
	 	...
	 	Tags: Wordpress, Buddypress, Facebook, Open Graph, Social Graph, Open Graph Protocol, Meta Tag, Social Graph Protocol, Social Plugins, Buddypress Facebook Sharer, 
	*/
...
-Tags: Facebook, Open Graph, Social Graph, Open Graph Protocol, Social Graph Protocol, CodeHooks, Code Hooks, Social, Social Plugins, Social Media
	...
	class SocialGraphProtocol {
		...
		function __construct() {
			...
			$this->textdomain = 'social-graph-protocol-buddypress-yr'; // Global variable to store name of my text domain

			load_plugin_textdomain( $this->textdomain, false, 'social-graph-protocol-buddypress-yr/i18n' ); // Used to load the textdomain in order to internationalize the plugin
...
//			include_once( ABSPATH . 'wp-content/plugins/social-graph-protocol-buddypress-yr/includes/meta-box.php' );
			include_once( dirname( __FILE__ ) . '/includes/meta-box.php' );  // YR 18 03 2016
			return;
...
//YR 190815-150116 start  *** for Buddypress activity
if ( function_exists( 'bp_get_activity_thread_permalink' ) ) {     // all actions only for buddypress
				$openGraphTagsJR = bp_get_activity_thread_permalink();
				$permalink_idJR = bp_get_activity_permalink_id(); 

				$position_activity = stripos($openGraphTagsJR, "activity/p/");
				if (  ($position_activity != false) and ($permalink_idJR != "") ) {
					$lengthJR = $position_activity + 11; 
					$openGraphTagsJR = substr($openGraphTagsJR,0,$lengthJR);
					$url1 = $openGraphTagsJR . $permalink_idJR . "/";
					
					// get redirect buddypress url
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL, $url1);
					curl_setopt($ch, CURLOPT_HEADER, 1);  // include headers
					curl_setopt($ch, CURLOPT_NOBODY, 1);  // not incl body text
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);  // not echo 
					...
					if (  $pos1 != false ) {
						...
						$url2_l = strlen($url2);
						$url2 = substr($url2,0,$url2_l-4);  // cut last 4 bytes
						$openGraphTagsJR = $url2;
						}
					$openGraphTags->attributes['og:url'] = $openGraphTagsJR;
					global $wpdb;
					$activitydescr = $wpdb->get_var( "SELECT content FROM wp_bp_activity WHERE id={$permalink_idJR}" );
//
//					$activitydescr = bp_get_activity_content_body();
					$symbrepl = array('\"', "»");
					$activitydescr = str_replace($symbrepl, '`', $activitydescr);
					$activitydescr = trim( substr( strip_shortcodes( strip_tags( str_ireplace( "\n", '', $activitydescr ) ) ), 0, 240 ) ); // Strip out any shortcodes, html elements and trim the content
					$openGraphTags->attributes['og:description'] = $activitydescr." ... ";
				} else {}
} else {}
//YR end	
